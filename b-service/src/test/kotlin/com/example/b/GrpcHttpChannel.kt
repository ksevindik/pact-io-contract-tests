...
class GrpcHttpChannel(private val host: String, private val port: Int) : Channel() {
    ...
    override fun <ReqT, RespT> newCall(
        ...
        val requestMarshaller: MethodDescriptor.Marshaller<Message> = this.jsonMarshaller(reqT as Message)
        val responseMarshaller: MethodDescriptor.Marshaller<Message> = this.jsonMarshaller(respT as Message)
        return object : ClientCall<ReqT, RespT>() {
            ...
            override fun sendMessage(message: ReqT) {
                ...
                val jsonRequest = asString(requestMarshaller.stream(message as Message))
                ...
                responseListener!!.onMessage(response as RespT)
    ...
    companion object {
        private val log = LoggerFactory.getLogger(GrpcHttpChannel::class.java)
    }
