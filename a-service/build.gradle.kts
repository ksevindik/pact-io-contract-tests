...
pact {
    ...
    serviceProviders {
        val s = create("a-service")
        s.stateChangeUrl = uri("http://localhost:8083/pact/stateChange").toURL()
...
/*
in order to execute canIDeploy task, you have to set -Ppacticipant=a-service -PpacticipantVersion=0.0.1-SNAPSHOT
from Run Configurations. Unfortunately, I was not able to provide those properties from within gradle build file.
tried
setProperty("pacticipant","${project.name}")
setProperty("pacticipantVersion","${project.version}")
both from project root and from within PactCanIDeployTask with no success.
 */
